...
function buildFocusedDocument(filterCss?: string) {
  const targetJson = JSON.stringify([
    { selector: TARGET_SELECTOR, role: 'background' },
  ]).replace(/</g, '\\u003c');
  const filterStyle = filterCss ? `filter: ${filterCss} !important;` : '';
...
[data-threeui-role="background"] { position: fixed !important; inset: 0 !important; width: 100% !important; height: 100% !important; max-width: none !important; max-height: none !important; z-index: 0 !important; opacity: 1 !important; pointer-events: none !important; ${filterStyle} }
</style>`;
...
export function FluidFieldBackground({
  ...
  const source = useMemo(() => buildFocusedDocument(filter), [filter]);
  ...
  return (
    <View style={[styles.nativeContainer, style]} pointerEvents="none">
      <WebView
        originWhitelist={['*']}
        source={{ html: source }}
