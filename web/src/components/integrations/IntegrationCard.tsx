...
import { Sparkles, ArrowRight, Share2, Layers } from "lucide-react";
  ...
  "group/button inline-flex shrink-0 items-center justify-center rounded-lg border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        ...
        outline:
          "border-white/10 bg-transparent hover:bg-white/5 hover:text-white aria-expanded:bg-white/5 aria-expanded:text-white",
        secondary:
          "bg-white/10 text-white hover:bg-white/15 aria-expanded:bg-white/10 aria-expanded:text-white",
        ghost:
          "hover:bg-white/5 hover:text-white aria-expanded:bg-white/5 aria-expanded:text-white",
        destructive:
...
export function Integration() {
  ...
  return (
    <div className="relative h-full w-full">
      ...
      {/* Center MindMesh Neural Core */}
      <div className="absolute top-1/2 left-1/2 z-20 flex -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-2xl border border-white/20 bg-black/90 p-2 shadow-2xl backdrop-blur-md">
        <div className="border border-white/10 p-2.5 rounded-xl bg-black/80 flex items-center justify-center">
          <img
            src="/mindmesh-icon.png"
            alt="MindMesh AI"
            width={36}
            height={36}
            className="size-8 sm:size-9 object-cover rounded-lg"
            onError={(e) => {
      ...
      {integrations.map((integration) => {
        ...
        return (
          <motion.div
            ...
            className="absolute z-10 flex h-9 w-9 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-xl border border-white/15 bg-[#0e0e18] shadow-lg sm:h-12 sm:w-12 text-white hover:border-cyan-400/50 hover:scale-110 transition-all duration-200"
          >
            <Icon className="h-4 w-4 sm:h-6 sm:w-6 text-white/90" />
          </motion.div>
        );
      })}
    </div>
...
export function VisualContainer({ children, className }: VisualContainerProps) {
  return (
    <div
      className={cn(
        "relative flex aspect-564/460 w-full items-center justify-center overflow-hidden rounded-t-2xl bg-[#090912] p-8 sm:aspect-564/410",
        className,
      )}
    >
      {/* Dots Background */}
...
export const IntegrationCard = ({
  ...
  return (
    <Card className="mx-auto flex w-full flex-col sm:max-w-141 rounded-3xl overflow-hidden p-0 ring-0 border border-white/10 bg-[#07070d] shadow-2xl">
      <VisualContainer>{visual}</VisualContainer>

      <CardContent className="p-6 sm:p-8 flex flex-col gap-6 sm:gap-8 bg-[#0a0a14] border-t border-white/5">
        <div className="flex flex-col gap-2">
          <div className="inline-flex items-center gap-2 text-cyan-400 text-xs font-mono tracking-widest uppercase">
            <Share2 className="w-3.5 h-3.5" />
            <span>NEURAL ECOSYSTEM BRIDGE</span>
          </div>
          <h3 className="text-xl sm:text-2xl font-serif tracking-tight text-white">
            {title}
          </h3>
          <p className="text-sm sm:text-base leading-relaxed text-white/60 font-sans">
            {description}
          </p>
        </div>
        <Button
          nativeButton={false}
          className="h-10 w-fit rounded-full px-6 bg-white text-black hover:bg-white/90 cursor-pointer font-sans font-medium text-xs sm:text-sm flex items-center gap-2 shadow-lg shadow-white/10 transition-transform hover:scale-105"
...
export function IntegrationSection() {
  return (
    <section id="integrations" className="relative w-full py-24 px-6 md:px-12 bg-black text-white border-t border-white/5 overflow-hidden">
      {/* Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[350px] bg-cyan-600/10 rounded-full blur-[140px] pointer-events-none" />

      <div className="mx-auto max-w-7xl flex flex-col items-center">
        {/* Section Header */}
        <div className="text-center mb-14 max-w-2xl">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full border border-white/10 bg-white/[0.03] text-xs font-mono tracking-widest text-cyan-400 uppercase mb-4 backdrop-blur-md">
            <Layers className="w-3.5 h-3.5" />
            <span>02.5 / UNIFIED WORKFLOW</span>
          </div>
          <h2 className="text-4xl md:text-5xl font-serif tracking-tight font-normal text-white leading-tight">
            Connects effortlessly. <br />
            <span className="italic font-light text-white/70">Lives wherever you build.</span>
          </h2>
          <p className="mt-4 text-white/50 text-sm md:text-base font-sans leading-relaxed">
            Ingest thoughts seamlessly from your design canvas, browser research, coding environment, and intelligence models into one persistent second brain.
          </p>
        </div>

        {/* The Animated Integration Card */}
        <div className="w-full flex items-center justify-center">
          <IntegrationCard
            visual={<Integration />}
            title="Unified Synaptic Ingestion"
            description="Whether brainstorming in Figma, reasoning with Claude, styling with Tailwind, or writing code, MindMesh weaves incoming insights into your personal knowledge base in real time."
            url="https://github.com/MdTowfikomer/MindMesh-AI"
          />
        </div>
      </div>
    </section>
